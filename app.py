...
def main():
    st.set_page_config(
        page_title="AI-Powered Data Cleaning Platform", page_icon=None, layout="wide"
    )
    ...
    with st.sidebar:
        ...
        current_step_idx = (
            steps.index(st.session_state.current_step.title())
            if st.session_state.current_step.title() in steps
            else 0
        )
        ...
        try:
            ...
            if isinstance(data, pd.DataFrame):
                ...
                with col2:
                    ...
                    st.metric(
                        "Size (MB)",
                        f"{data.memory_usage(deep=True).sum() / 1024 / 1024:.2f}",
                    )
            else:
                st.text_area(
                    "Text Data Preview",
...
def handle_schema_management(logger, llm_client):
    ...
    if st.session_state.schema is None:
        with st.spinner("Detecting schema..."):
            st.session_state.schema = schema_manager.detect_schema(
            ...
            with col2:
                new_dtype = st.selectbox(
                    ...
                    index=[
                        ...
                        "bool",
                        "category",
                    ].index(col_info.get("suggested_dtype", col_info["dtype"]))
                    if col_info.get("suggested_dtype", col_info["dtype"])
                    in [
                        "object",
                        "int64",
                        "float64",
        ...
        if nl_instruction:
            with st.spinner("Processing natural language instruction..."):
                try:
                    ...
                    strategy = llm_client.summarize_strategy(
                        generated_code,
                        f"Schema changes for {len(st.session_state.schema)} columns",
                    )
                    ...
                    updated_schema = schema_manager.process_nl_schema_change(
                        st.session_state.schema, nl_instruction
                    )
        ...
        if hasattr(st.session_state, "proposed_schema"):
            ...
            for col_name in st.session_state.schema.keys():
                if col_name in st.session_state.proposed_schema:
                    ...
                    new_type = st.session_state.proposed_schema[col_name].get(
                        "dtype", "unknown"
                    )
            ...
            if changes_found:
                ...
                with col1:
                    if st.button(
                        "✅ Approve & Apply", key="approve_nl_changes", type="primary"
                    ):
                ...
                with col2:
                    if st.button("Modify Instruction", key="modify_nl"):
    ...
    with col2:
        if st.button(
            "Proceed to Data Analysis", type="primary", key="schema_to_analysis_btn"
        ):
            st.session_state.current_step = "analysis"
            st.rerun()
...
def handle_data_analysis(logger, llm_client):
    ...
    with st.spinner("Analyzing data quality..."):
        try:
            ...
            quality_report = data_analyzer.generate_quality_report(
                st.session_state.data
            )
            ...
            if llm_client and llm_client.client:
                try:
                    enhanced_analysis = data_analyzer.enhanced_llm_analysis(
                        st.session_state.data, st.session_state.schema, quality_report
                    )
                except Exception as e:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Overview", "Visualizations", "Detailed Report", "AI Insights"]
    )
    ...
    with tab1:
        ...
        with col1:
            st.metric(
                "Data Quality Score",
                f"{quality_report.get('quality_score', 0):.1f}/100",
            )
        ...
        with col1:
            st.metric(
                "Missing Values",
                f"{quality_report['missing_values']['missing_percentage']:.1f}%",
            )

        with col2:
            st.metric(
                "Duplicates",
                f"{quality_report['duplicates']['duplicate_percentage']:.1f}%",
            )
        ...
        with st.expander("Missing Values Analysis", expanded=True):
            ...
            if missing_info["columns_with_missing"]:
                ...
                if missing_info["problematic_columns"]:
                    st.warning(
                        f"Problematic columns (>{50}% missing): {', '.join(missing_info['problematic_columns'])}"
                    )
    ...
    with tab4:
        ...
        st.text_area(
            "Analysis Summary", enhanced_analysis.get("summary", ""), height=150
        )
    ...
    st.markdown("---")
    if st.button(
        "Proceed to Data Correction",
        type="primary",
        key="analysis_to_correction_btn",
    ):
        st.session_state.current_step = "correction"
        st.rerun()
...
def handle_data_correction(logger, llm_client):
    ...
    with st.expander(
        "Correction Setup",
        expanded=not st.session_state.correction_state["generated"],
    ):
        ...
        if st.button(
            ...
            with st.spinner("Generating correction strategy..."):
                try:
                    correction_code, strategy_summary = (
                        data_corrector.generate_correction_code(
                            st.session_state.data,
    ...
    if st.session_state.correction_state["generated"]:
            ...
            with col1:
                ...
                if st.button(
                    ...
                    with st.spinner("Executing correction..."):
                        try:
                            corrected_data, execution_log = (
                                data_corrector.execute_correction_code(
                                    st.session_state.data,
                            ...
                            logger.log(
                                "Correction execution error",
                                {"error": str(e), "traceback": traceback.format_exc()},
                            )

            with col2:
                if st.button("Modify Code", key="modify_code_button"):
        ...
        if (
            hasattr(st.session_state, "show_code_editor")
            and st.session_state.show_code_editor
        ):
    ...
    if st.session_state.correction_state["executed"]:
        ...
        if st.session_state.correction_state["show_proceed"]:
            st.markdown("---")
            if st.button(
                "Proceed to Finalization", type="primary", key="proceed_to_final_btn"
            ):
    ...
    with st.spinner("Executing correction..."):
        try:
            corrected_data, execution_log = data_corrector.execute_correction_code(
                st.session_state.data, st.session_state.correction_code
            )

            if execution_log["success"]:
                st.session_state.data = corrected_data
                st.session_state.correction_executed = (
                    True  # Add flag for successful execution
                )
                logger.log("Data correction executed", execution_log)
                ...
                st.write(
                    f"Correction complete: {st.session_state.get('correction_complete', False)}"
                )
                ...
                with col2:
                    st.info(
                        f"**Memory:** {st.session_state.data.memory_usage(deep=True).sum() / 1024 / 1024:.2f} MB"
                    )
                ...
                if st.button(
                    ...
                    type="primary",
                    key="correction_to_finalization_btn",
                ):
                ...
                st.info(
                    "Try modifying the code or regenerating with different instructions."
                )
...
def handle_finalization(logger, llm_client):
    ...
    if st.button(
        "Generate Final Pipeline", type="primary", key="generate_pipeline_btn"
    ):
        with st.spinner("Generating complete pipeline..."):
            try:
                ...
                logger.log(
                    "Pipeline generated", {"pipeline_length": len(pipeline_code)}
                )
    ...
    if hasattr(st.session_state, "pipeline_code") and st.session_state.pipeline_code:
        ...
        with col1:
            original_shape = (
                st.session_state.original_data.shape
                if hasattr(st.session_state, "original_data")
                and hasattr(st.session_state.original_data, "shape")
                else (0, 0)
            )
        ...
        with col4:
            reduction_pct = (
                (rows_removed / original_shape[0] * 100) if original_shape[0] > 0 else 0
            )
                    ...
                    try:
                        ...
                        with pd.ExcelWriter(output, engine="openpyxl") as writer:
                            ...
                            if hasattr(
                                ...
                                st.session_state.original_data.to_excel(
                                ...
                                quality_summary = pd.DataFrame(
                                    {
                                        ...
                                        "Value": [
                                            st.session_state.quality_report.get(
                                                "quality_score", 0
                                            ),
                                            st.session_state.quality_report.get(
                                                "missing_values", {}
                                            ).get("missing_percentage", 0),
                                            st.session_state.quality_report.get(
                                                "duplicates", {}
                                            ).get("duplicate_percentage", 0),
                                            st.session_state.quality_report.get(
                                                "outliers", {}
                            ...
                            summary_data = {
                                ...
                                "Value": [
                                    st.session_state.original_data.shape[0]
                                    if hasattr(st.session_state, "original_data")
                                    and hasattr(st.session_state.original_data, "shape")
                                    else 0,
                                    ...
                                    - st.session_state.data.shape[0],
                                    st.session_state.original_data.shape[1]
                                    if hasattr(st.session_state, "original_data")
                                    and hasattr(st.session_state.original_data, "shape")
                                    else 0,
...
"""
        ...
        st.info(
            "All files are ready for download. The pipeline script can be used to replicate this cleaning process on new data."
        )
    ...
    if st.button("Start New Session", key="new_session_btn"):
        ...
        st.info(
            "Click 'Generate Final Pipeline' to complete the data cleaning process and enable downloads."
        )
...
def display_logs(logger):
    ...
    with col2:
        action_filter = st.text_input(
            "Filter by Action", placeholder="Enter keyword", key="log_action_filter"
        )
    ...
    with st.expander("Performance Metrics"):
        ...
        with col3:
            ...
            if metrics["average_execution_time"] > 0:
                st.metric(
                    "Avg Execution Time", f"{metrics['average_execution_time']:.2f}s"
                )
            ...
            if st.button("Generate Excel Report", key="gen_excel"):
                with st.spinner("Generating Excel report..."):
                    ...
                    with pd.ExcelWriter(output, engine="openpyxl") as writer:
                        ...
                        # Original data (if available)
                        if hasattr(st.session_state, "original_data") and isinstance(
                            st.session_state.original_data, pd.DataFrame
                        ):
                            st.session_state.original_data.to_excel(
                                writer, sheet_name="Original Data", index=False
                        ...
                        summary_data = {
                            ...
                            "Value": [
                                st.session_state.original_data.shape[0]
                                if hasattr(st.session_state.original_data, "shape")
                                else 0,
                                st.session_state.data.shape[0],
                                (
                                    st.session_state.original_data.shape[0]
